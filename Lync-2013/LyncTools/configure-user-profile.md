...
Para configurar perfis de usuário, execute a ferramenta de configuração de carregamento do Lync Server 2013 (UserProfileGenerator.exe) e preencha cada uma das guias. UserProfileGenerator.exe gera um diretório para cada um dos computadores cliente que você precisa para executar a simulação. Cada diretório de cliente também vem com um script para iniciar todas as instâncias da ferramenta de desempenho e stress (LyncPerfTool.exe) do Lync Server 2013.
...
4.  Em **índice de início do usuário**, clique ou digite o índice do primeiro usuário a ser configurado. Você pode configurar intervalos diferentes para tipos ou níveis diferentes de carga, mas você deve executar UserProfileGenerator.exe uma vez pelo intervalo que deseja configurar.
