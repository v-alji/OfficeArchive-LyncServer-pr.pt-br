...
Cada servidor de borda requer um certificado público na interface entre a rede de perímetro e a Internet, e o nome alternativo do sujeito do certificado deve conter os nomes externos dos FQDNs (nomes de domínio totalmente qualificados do serviço de borda de acesso e Web de Webconferência).
...
5.  Na página **repositório de certificados** , selecione o certificado público que você solicitou e importou para a interface externa do servidor de borda.
    ...
    > [!NOTE]  
    > Se o certificado que você solicitou e importado não estiver na lista, um dos métodos de solução de problemas será verificar se o nome da entidade e os nomes alternativos da entidade do certificado atendem a todos os requisitos do certificado e, se você importou manualmente o certificado e a cadeia de certificados em vez de usar os procedimentos anteriores, se o certificado estiver no repositório de certificados correto , não o usuário ou o repositório de certificados do serviço.
