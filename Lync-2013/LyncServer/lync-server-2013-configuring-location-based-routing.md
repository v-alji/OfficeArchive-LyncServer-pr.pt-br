...
Lync Server 2013 CU1, Location-Based o roteamento é um recurso do Enterprise Voice. Location-Based o roteamento é um recurso de gerenciamento de chamadas que controla como as chamadas são roteadas pelo Lync Server 2013 CU1. Ele impõe restrições sobre se as chamadas podem ser roteadas para destinos PBX ou PSTN com base na localização do chamador do Lync. Location-Based roteamento aplica regras de autorização de chamada a chamadas PSTN com base no local de rede do chamador. O local do chamador é determinado com base no site de rede associado à sub-rede da rede na qual o chamador está conectado. Configurar o roteamento Location-Based requer primeiro a implantação do Enterprise Voice e, em seguida, a configuração de regiões de rede, sites e sub-redes. Isso configura a base para habilitar o roteamento de Location-Based.

Antes de implantar o roteamento Location-Based, você deve primeiro implantar o Enterprise Voice e configurar regiões de rede, sites e associar sub-redes de rede a seus sites de rede. Uma vez concluído, você pode configurar o roteamento de Location-Based. Para ver as etapas sobre como configurar regiões de rede, sites e sub-redes, consulte [implantando recursos avançados do Enterprise Voice no Lync Server 2013](lync-server-2013-deploying-advanced-enterprise-voice-features.md)

Esta seção orienta você na configuração do roteamento Location-Based usando o exemplo a seguir como ilustração.
