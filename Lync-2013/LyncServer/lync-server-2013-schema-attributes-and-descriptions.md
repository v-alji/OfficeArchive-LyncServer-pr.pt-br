...
<td><p>Esse atributo é um valor de cadeia de caracteres de valor único. O valor desse atributo, quando presente, representa o FQDN do domínio do pool se o administrador quiser criar um pool de front-end com um FQDN que não esteja em conformidade com a estrutura de domínio do Active Directory na qual o pool de front-ends é criado (por exemplo, um namespace SIP desassociado do espaço para nome do sistema de nomes de domínio (DNS)).</p>
