...
As configurações de arquivamento podem ser verificadas usando o Windows PowerShell e o Get-CsArchivingConfiguration cmdlet:
...
As informações recebidas do Get-CsArchivingPolicy serão parecidas com isto:
...
O cmdlet Get-CsCdrConfiguration retorna informações parecidas com isso para cada coleção de definições de configuração de CDR:
...
Informações semelhantes podem ser retornadas para monitoramento de QoE usando o cmdlet Get-CsQoEConfiguration. Por exemplo, esse comando retorna informações sobre a coleção global de definições de configuração de QoE:
